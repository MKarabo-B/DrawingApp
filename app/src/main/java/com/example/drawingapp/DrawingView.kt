...
class DrawingView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when (event.action) {
            //this event will occur whe the user puts the finger on the screen
            MotionEvent.ACTION_DOWN -> {
                drawPath.color = color
                drawPath.brushThickness = brushSize

                drawPath.reset()//resets path before we set initial point
                drawPath.moveTo(touchX, touchY)
            }

            //the event will occur when the user starts to move their finger
            //this will occur continually until user removes finger form screen
            MotionEvent.ACTION_MOVE -> {
                drawPath.lineTo(touchX, touchY)
            }

            //this event will occur when the user will pick up their finger from the screen
            MotionEvent.ACTION_UP -> {
                // Add the finished path to the list of paths
                paths.add(drawPath)
                // Create a new path for the next stroke
        ...
        invalidate() // Refresh the view to show changes
        return true
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Draw the current path (active stroke)
        if (!drawPath.isEmpty) {
